...
>
> <img src="./cdyfzqla.png"
> style="width:5.76319in;height:5.77014in" />和语法决策都旨在让程序员的生活更轻松。
>
...
> 件顶部使用以下命令声明：
>
> <img src="./xlgzb0oi.png"
> style="width:5.76319in;height:5.77014in" />package \<packagename\>
>
...
>
> Goroutine线程。Goroutine 线程比标准线程更轻量级，大多数 Golang 程序
>
> 同时使用数千个 Goroutine。
>
> 9
...
>
> <img src="./rfm4jqev.png"
> style="width:5.76319in;height:5.77014in" />查。
>
> package main
>
> func main() {
...
>
> <img src="./t5j3suzq.png"
> style="width:5.76319in;height:5.77014in" />make(T,
> args)函数的目的和new(T)不同 仅仅用于创建slice, map,
>
> channel 而且返回类型是实例
>
> **15、Printf()，Sprintf()，FprintF()** **都是格式化输出，有什么不**
>
> **同？**
>
...
> 13
>
> **17、Go** **语言当中值传递和地址传递（引用传递）如何运用？有什**
>
> **么区别？举例说明**
>
...
> 不可相互修改。
>
> <img src="./vjg4gigj.png"
> style="width:5.76319in;height:5.77014in" />2.
...
> Go 中切片扩容的策略是这样的：
>
> ⚫ 首先判断，如果新申请容量大于2倍的旧容量，最终容量就是新申请的容
>
> 量
>
...
>
> <img src="./2zllu4t0.png"
> style="width:5.76319in;height:5.77014in" />**25、Golang** **Map**
> **底层实现**
>
...
>
> <img src="./1oqyb02r.png"
> style="width:5.76319in;height:5.77014in" /><img src="./jfhnm1yr.png"
> style="width:5.76389in;height:1.81181in" />上图展示的是一个缓冲区为8
> 的channel buffer，recvx 指向最早被读取的数
>
...
> **正常模式（非公平锁）**
>
> 正常模式下，所有等待锁的goroutine按照FIFO（先进先出）顺序等待。唤醒
>
> 的goroutine不会直接拥有锁，而是会和新请求goroutine竞争锁。新请求的
>
> goroutine更容易抢占：因为它正在CPU上执行，所以刚刚唤醒的goroutine
>
> 19
>
> 有很大可能在锁竞争中失败。在这种情况下，这个被唤醒的goroutine会加入
>
> 到等待队列的前面。
>
> **饥饿模式（公平锁）**
>
> 为了解决了等待goroutine队列的长尾问题
>
> 饥饿模式下，直接由unlock把锁交给等待队列中排在第一位的goroutine (队
>
> <img src="./lo55tjss.png"
> style="width:5.76319in;height:5.77014in" />头)，同时，饥饿模式下，新进来的goroutine不会参与抢锁也不会进入自旋状
>
> 态，会直接进入等待队列的尾部。这样很好的解决了老的goroutine一直抢不
>
> 到锁的场景。
>
> 饥饿模式的触发条件：当一个goroutine等待锁时间超过1毫秒时，或者当前
>
> 队列只剩下一个goroutine的时候，Mutex切换到饥饿模式。
>
> **总结**
>
...
> **5、RWMutex** **注意事项**
>
> <img src="./dly3ls4i.png"
> style="width:5.76319in;height:5.77014in" />⚫ RWMutex
> 是单写多读锁，该锁可以加多个读锁或者一个写锁
>
...
>
> <img src="./hsmbuyqk.png"
> style="width:5.76319in;height:5.77014in" />func (c \*Cond) Broadcast()
>
...
> }
>
> ... make use of condition ... c.L.Unlock()
>
> <img src="./lnrq2vg5.png"
> style="width:5.76319in;height:5.77014in" />**9、WaitGroup** **用法**
>
> 一个 WaitGroup 对象可以等待一组协程结束。使用方法是：
>
> 1\. main协程通过调用 wg.Add(delta int) 设置worker协程的个数，然后创
>
> 建worker协程；
>
> 2.worker协程执行结束以后，都要调用 wg.Done()；
>
> 3.main协程调用 wg.Wait() 且被block，直到所有worker协程全部执行结束
>
> 后返回。
>
> **10、WaitGroup** **实现原理**
>
> ⚫ WaitGroup主要维护了2个计数器，一个是请求计数器 v，一个是等待计数
>
> 器 w，二者组成一个64bit的值，请求计数器占高32bit，等待计数器占低
>
> 32bit。
>
> ⚫ 每次Add执行，请求计数器v加1，Done方法执行，等待计数器减1，v为
>
> 0时通过信号量唤醒Wait()。
>
> **11、什么是** **sync.Once**
...
> **12、什么操作叫做原子操作**
>
> <img src="./5hdzwvg2.png"
> style="width:5.76319in;height:5.77014in" />原子操作即是进行过程中不能被中断的操作，针对某个值的原子操作在被进行
>
...
> **14、什么是** **CAS**
>
> CAS的全称为Compare And Swap，直译就是比较交换。是一条CPU的原子指
>
...
> 24
>
> 假设包含3个参数内存位置(V)、预期原值(A)和新值(B)。V表示要更新变量的
>
> 值，E表示预期值，N表示新值。仅当V值等于E值时，才会将V的值设为N，
>
...
> 以发现其他线程对于当前线程的干扰。
>
> <img src="./uw5gpovh.png"
> style="width:5.76319in;height:5.77014in" />**15、sync.Pool**
> **有什么用**
>
...
> **2、GMP** **指的是什么**
>
> G（Goroutine）：我们所说的协程，为用户级的轻量级线程，每个Goroutine
>
> 对象中的sched保存着其上下文信息。
>
> 25
>
> M（Machine）：对内核级线程的封装，数量对应真实的CPU数（真正干活的对
>
> 象）。
>
> P（Processor）：即为G和M的调度对象，用来调度G和M之间的关联关系，
> 其数量可通过GOMAXPROCS()来设置，默认为核心数。
>
> **3、1.0** **之前** **GM** **调度模型**
>
> <img src="./ynxgcxme.png"
> style="width:5.76319in;height:5.77014in" />调度器把G都分配到M上，不同的G在不同的M并发运行时，都需要向系统申
>
...
> **4、GMP** **调度流程**
>
> <img src="./hgsg3u5h.png"
> style="width:5.76319in;height:5.77014in" /><img src="./1jc5yb1s.png"
> style="width:5.70375in;height:3.27361in" />⚫
> 每个P有个局部队列，局部队列保存待执行的goroutine（流程2），当M
>
> 绑定的P的的局部队列已经满了之后就会把goroutine放到全局队列（流
>
> 程2-1）
>
> ⚫ 每个P和一个M绑定，M是真正的执行P中goroutine的实体（流程3），
>
> M从绑定的P中的局部队列获取G来执行
>
...
>
> <img src="./jfyypowx.png"
> style="width:5.76319in;height:5.77014in" />清理线程→继续找新的Goroutine执行）
>
> **6、GMP** **中** **hand** **off** **机制**
>
...
> **8、基于信号的抢占式调度**
>
> 在任何情况下，Go运行时并行执行（注意，不是并发）的 goroutines 数量是
>
...
> **11、三色标记原理**
>
> <img src="./h2xpct3y.png"
> style="width:5.76319in;height:5.77014in" /><img src="./tg3pvt2o.png" style="width:4.00139in;height:3.35in" />我们首先看一张图，大概就会对
> 三色标记法有一个大致的了解：
>
> 原理：
>
...
> ⚫ 循环步骤3，知道灰色集合中没有对象
>
> ⚫ 步骤4结束后，白色集合中的对象就是不可达对象，也就是垃圾，进行回
>
> 收
>
> 30
...
> 那么我们考虑一下，下面的情况。
>
> <img src="./pp1tfyg0.png"
> style="width:5.76319in;height:5.77014in" /><img src="./tnfu344u.png"
> style="width:5.70417in;height:4.48125in" />我们在进行三色标记中扫描灰色集合中，扫描到了对象A，并标记了对象A的
>
...
> 31
>
> <img src="./d3ot03vs.png"
> style="width:5.76319in;height:5.77014in" /><img src="./j1oov31g.png"
> style="width:5.70417in;height:4.71319in" />这样会不会导致E对象就扫描不到了，而被误认为
> 为白色对象，也就是垃圾
>
...
> 活过这一轮，在下一轮GC中才被清理掉。
>
> <img src="./l4tw5tsw.png"
> style="width:5.76319in;height:5.77014in" />**15、混合写屏障**
>
...
>
> <img src="./32owo5ex.png"
> style="width:5.76319in;height:5.77014in" />障
>
...
<img src="./l4rlyi5r.png"
style="width:5.76319in;height:5.77014in" /><img src="./jyt4ka1h.png"
style="width:5.70417in;height:5.78389in" />
...
> **2、说说微服务架构的优势**
>
> <img src="./beockw5m.png"
> style="width:5.76319in;height:5.77014in" />优势 说明
>
...
> 以下是设计微服务的最佳实践：
>
> <img src="./05w04lmb.png"
> style="width:5.76319in;height:5.77014in" /><img src="./u2c4j2rf.png"
> style="width:5.70417in;height:3.44444in" />图6：设计微服务的最佳实践 –
> 微服务访谈问题
>
> **5、微服务架构如何运作？**
>
...
> 支持单个可部署单元
>
> <img src="./5nl0zfsp.png"
> style="width:5.76319in;height:5.77014in" /><img src="./kxc030q3.png"
> style="width:4.56944in;height:2.92361in" />允许经常发布软件
>
> 确保每项服务的安全性
>
> 多个服务是并行开发和部署的
...
> **8、在使用微服务架构时，您面临哪些挑战？**
>
> <img src="./ao2o4yc4.png"
> style="width:5.76319in;height:5.77014in" />开发一些较小的微服务听起来很容易，但开发它们时经常遇到的挑战如下。
>
...
> 您可以列出微服务的特征，如下所示：
>
> <img src="./dz4z0vib.png"
> style="width:5.76319in;height:5.77014in" /><img src="./qrl4zpp4.png"
> style="width:5.70361in;height:1.75417in" /><img src="./vfpj5roj.png"
> style="width:5.70278in;height:1.9875in" />图7：微服务的特征 –
> 微服务访谈问题
>
> **11、什么是领域驱动设计？**
>
...
> **12、为什么需要域驱动设计（DDD）？**
>
> <img src="./ply3fjjn.png"
> style="width:5.76319in;height:5.77014in" /><img src="./0uujtflo.png"
> style="width:5.70403in;height:3.38611in" />图 9：我们需要 DDD 的因素 –
> 微服务面试问题
>
> **13、什么是无所不在的语言？**
>
...
> 内聚力和低耦合性。
>
> <img src="./fd10akae.png"
> style="width:5.76319in;height:5.77014in" />**16、什么是** **REST**
> **/** **RESTful** **以及它的用途是什么？**
>
...
>
> <img src="./xn3lukpm.png"
> style="width:5.76319in;height:5.77014in" />特性改变集为目标的新软件版本，代替发布一个大特性改变集的新软件版本的
>
...
> 更快得解决问题
>
> <img src="./rs2o3tlx.png"
> style="width:5.76319in;height:5.77014in" />商业优势:
>
> 更快交付的特性
>
> 更稳定的操作系统环境
...
> 44
>
> **6、Dockerfile** **配置文件中的** **COPY** **和** **ADD**
> **指令有什么不同？**
>
...
>
> <img src="./bqkexrze.png"
> style="width:5.76319in;height:5.77014in" />单，而 ADD
> 有一些其它特性功能（诸如，本地归档解压和支持远程网址访问
>
...
>
> <img src="./sb0z5vil.png"
> style="width:5.76319in;height:5.77014in" />级的资源中心，成为用户组或团队协作开发中保证自动化开发流程的有效技术
>
> 途径。
>
...
>
> <img src="./wuivejfo.png"
> style="width:5.76319in;height:5.77014in" />中， FROM 是第一条指令。
>
...
>
> <img src="./l4lxurth.png"
> style="width:5.76319in;height:5.77014in" />器在许多方面，与虚拟机非常相似，但容器的运行不需要虚拟管理软件的运
>
> 行。
>
> **15、Docker** **Image** **和** **Docker** **Layer** **（层)**
> **有什么不同？**
>
...
>
> <img src="./ycnr3wy0.png"
> style="width:5.76319in;height:5.77014in" />实质的效果是，虚拟化技术允许你在一个硬件平台下运行
> 2 个完全不同的操作
>
...
>
> <img src="./nf3stwof.png"
> style="width:5.76319in;height:5.77014in" />**18、Docker**
> **群（Swarm）是什么？**
>
...
> 孤儿卷存在很大问题。
>
> <img src="./xl12cxhp.png"
> style="width:5.76319in;height:5.77014in" /><img src="./v0mzohr0.png"
> style="width:5.70333in;height:1.28403in" />**21、什么是半虚拟化（Paravirtualization）？**
>
...
> 51
>
> <img src="./ozgl0zak.png"
> style="width:5.76319in;height:5.77014in" /><img src="./imbkswt5.png"
> style="width:5.70319in;height:2.03889in" />⚫
...
>
> **23、请解释一下** **docerfile** **配置文件中的** **ONBUILD**
> **指令的用途**
>
> **含义？**
>
...
>
> <img src="./xtobbtdw.png"
> style="width:5.76319in;height:5.77014in" /><img src="./m5ywnxx2.png"
> style="width:5.7025in;height:1.93056in" />是以整体或固化的方式导入自有映像中的，自有映像是不能对这个过程进行自
>
...
>
> <img src="./pnwsj3kr.png"
> style="width:5.76319in;height:5.77014in" />在主机的 hostfolder
> 目录下。 使用网络文件系统（nfs）与此类似。 那样您
>
> 就可以运行您的容器在任何主机上且其运行状态数据被保存在网络文件系统
>
> 上。
>
> **25、在** **Windows** **系统上可以运行原生的** **Docker**
> **容器吗？**
>
...
>
> <img src="./eajjylrj.png"
> style="width:5.76319in;height:5.77014in" />运行 Docker 容器都使用
> Linux 虚拟机（VMs） 技术， Docker 工具箱使用的
>
...
> 55
>
> <img src="./m4tpuozc.png"
> style="width:5.76319in;height:5.77014in" /><img src="./mu4ue2mr.png"
> style="width:5.70389in;height:2.9868in" />图上文字说明：
> 运行中的容器是隔离的。准确地说， 各容器共享操作系统内
>
> 核及操作系统 API。
>
...
> 一些修改建议：
>
> <img src="./4wvbriok.png"
> style="width:5.76319in;height:5.77014in" />移除应用代码中对任何固定存储卷的绑定，由于代码驻留在容器内部，而不能
>
> 从外部进行修正。
>
> 绑定应用端口到主机上的不同端口
...
> 重连数据库， 直至其连接到数据库为止。
>
> <img src="./cvbkcgpe.png"
> style="width:5.76319in;height:5.77014in" />最佳的解决方案是在您的应用代码中检查是否有应对意外的发生，无论是任何
>
> 原因导致的启动或连接失效都应考虑在内。
>
> **Redis**
...
> 败完全不执行。单个操作是原子性的。多个操作也支持事务，即原子性， 通过
> MULTI 和 EXEC 指令包起来。
>
> 58
>
...
>
> <img src="./tmc3zevu.png"
> style="width:5.76319in;height:5.77014in" />的同
> 时对程序员透明，无需进行额外的抽象。
>
...
> ⚫ Redis 可以持久化其数据
>
> <img src="./0ofyyhok.png"
> style="width:5.76319in;height:5.77014in" />**5、Memcache** **与**
> **Redis** **的区别都有哪些？**
>
...
> 协议不一样。 Redis 直接自己构建了 VM 机制 ，因为一般的系统调用系统 函
> 数的话，会浪费一定的时间去移动和请求。
>
> **6、Redis** **是单进程单线程的？**
>
...
> ⚫ 容灾性好，一个文件可以保存到安全的磁盘。
>
> <img src="./hkuoqce4.png"
> style="width:5.76319in;height:5.77014in" />⚫ 性能最大化，fork
> 子进程来完成写操作，让主进程继续处理命令，所以
>
...
> 步一
>
> <img src="./axvrm23s.png"
> style="width:5.76319in;height:5.77014in" />（3）为了主从复制的速度和连接的稳定性，Master
> 和 Slave 最好在同一个局 域网
>
> （4）尽量避免在压力很大的主库上增加从
>
> （5）主从复制不要用图状结构，用单向链表结构更为稳定，即：Master
> \<-Slave1\<- Slave2 \<- Slave3…这样的结构方便解决单点故障问题，实现
> Slave 对 Master 的替换。如果 Master 挂了，可以立刻启用 Slave1 做
> Master，其 他不变。
>
> **10、Redis** **过期键的删除策略？**
>
...
>
> <img src="./zpwfdhvb.png"
> style="width:5.76319in;height:5.77014in" />频率低，则使用 allkeys-lr
>
...
> 内存中，磁盘 I/O 速度为严重影响 Redis 的性能。在内存越来越便宜的今
...
> 有因果相关性。使用 Redis-benchmark 进行压测的时候可以发现影响 Redis 的
> QPS 峰值的一个重要因素是 pipeline 批次指令的数目。
>
> 63
>
> **15、是否使用过** **Redis** **集群，集群的原理是什么？**
>
> ⚫ Redis Sentinal 着眼于高可用，在 master 宕机时会自动将 slave 提升 为
> master，继续提供服务。
>
> ⚫ Redis Cluster 着眼于扩展性，在单个 Redis 内存不足时，使用 Cluster
> 进行分片存储。
>
> <img src="./nqflqxnj.png"
> style="width:5.76319in;height:5.77014in" />**16、Redis**
> **集群方案什么情况下会导致整个集群不可用？**
>
...
>
> <img src="./5bi4syo5.png"
> style="width:5.76319in;height:5.77014in" />**21、Redis**
> **集群的主从复制模型是怎样的？**
>
...
> **27、怎么理解** **Redis** **事务？**
>
> <img src="./oncs02vp.png"
> style="width:5.76319in;height:5.77014in" />（1）事务是一个单独的隔离操作：事务中的所有命令都会序列化、按顺序地执
...
> **31、Redis** **回收进程如何工作的？**
>
> <img src="./xyxi3vyv.png"
> style="width:5.76319in;height:5.77014in" />一个客户端运行了新的命令，添加了新的数据。Redi
> 检查内存使用情况， 如 果大于maxmemory 的限制,
> 则根据设定好的策略进行回收。一个新的命令被 执
...
>
> 例至少存放了2亿 5千万的 keys。我们正在测试一些较大的值。任何 list、
>
...
> 67
>
> **35、MySQL** **里有** **2000w** **数据，Redis** **中只存** **20w**
> **的数据，如**
>
> **何保证** **Redis** **中的数据都是热点数据？Redis**
> **内存数据集大小上升**
>
> **到一定大小的时候，就会施行数据淘汰策略。**
>
...
>
> <img src="./pywcz1pr.png"
> style="width:5.76319in;height:5.77014in" />volatile-lru：从已设置过期时间的数据集（server.db\[i\].expires）中挑选
>
> 最近最少使用的数据淘汰
>
> volatile-ttl：从已设置过期时间的数据集（server.db\[i\].expires）中挑选
...
> **队列**
>
> <img src="./g1za2geo.png"
> style="width:5.76319in;height:5.77014in" />Redis
> 在内存存储引擎领域的一大优点是提供 list 和 set 操作，这使得 Redis
> 能作为一个很好的消息队列平台来使用。Redis 作为队列使用的操作，
...
> **么？**
>
> <img src="./wrk3us54.png"
> style="width:5.76319in;height:5.77014in" />如果大量的 key
> 过期时间设置的过于集中，到过期的那个时间点，Redis 可能
>
...
> 70
>
> 到这里，面试官暗地 里已经对你竖起了大拇指。但是他不知道的是此刻你却竖
> 起了中指，在椅子背后。
>
...
> 记了释放。
>
> <img src="./3gl0wewn.png"
> style="width:5.76319in;height:5.77014in" />这时候对方会告诉你说你回答得不错，然后接着问如果在
> setnx 之后执行
>
...
>
> <img src="./1mg2o2ui.png"
> style="width:5.76319in;height:5.77014in" />定位到要查找的行;而 MyISAM
> 可以直接定位到数据所在的内存地址，可以
>
> 直接找到数据。
>
...
>
> ⚫ MyISAM索引的叶子节点存储的是行数据地址，需要再寻址一次才能得到数
>
> 据。
>
> ⚫ InnoDB非主键索引的叶子节点存储的是主键和其他带索引的列数据，因此
>
> 查询时做到覆盖索引会非常高效。
>
> **6、什么是索引？**
...
>
> 构，以协助快速查询、更新数据库表中数据。索引的实现通常使用B树及其变
>
> 种B+树。
>
...
>
> ⚫ 通过使用索引，可以在查询的过程中，使用优化隐藏器，提高系统的性
>
> <img src="./o3kraaua.png"
> style="width:5.76319in;height:5.77014in" />能。 索引的缺点
>
...
>
> ⚫ 可以通过ALTER TABLE table_name ADD INDEX index_name (column);创建
>
> 普通索引
>
...
> 是目前搜索引擎使用的一种关键技术。
>
> ⚫ 可以通过ALTER TABLE table_name ADD FULLTEXT (column);创建全文索
>
> 引。
>
> **9、MySQL** **中有哪几种锁？**
>
> <img src="./tp3fny5s.png"
> style="width:5.76319in;height:5.77014in" />⚫
...
>
> ⚫ char 列长度固定为创建表时声明的长度，长度值范围是1 到 255
>
> 75
>
...
>
> <img src="./krb5jxat.png"
> style="width:5.76319in;height:5.77014in" />主键也是候选键。按照惯例，候选键可以被指定为主键，并且可以用于任何外
>
> 键引用。
>
> **13、如何在** **Unix** **和** **MySQL** **时间戳之间进行转换？**
>
> UNIX_TIMESTAMP是从MySQL 时间戳转换为Unix时间戳的命令 FROM_UNIXTIME
>
> 是从Unix时间戳转换为MySQL时间戳的命令。
>
> **14、MyISAM** **表类型将在哪里存储，并且还提供其存储格式？**
>
...
> 9999999.99。
>
> <img src="./zbg0gtxr.png"
> style="width:5.76319in;height:5.77014in" />**16、创建索引时需要注意什么？**
>
> ⚫ 非空字段：应该指定列为NOT NULL，除非你想存储NULL。在MySQL中，含
>
> 有空值的列很难进行查询优化，因为它们使得索引、索引的统计信息以及
>
> 比较运算更加复杂。应该用0、一个特殊的值或者一个空串代替空值；
>
...
>
> 改时，索引本身也会被修改。 这意味着每条记录的INSERT，DELETE，UPDATE
>
> 将为此多付出4，5 次的磁盘I/O。 因为索引需要额外的存储空间和处理，那
>
> 些不必要的索引反而会使查询反应时间变慢。使用索引查询不一定能提高查询
>
...
>
> <img src="./piq3swsu.png"
> style="width:5.76319in;height:5.77014in" />数量是成正比的。
>
...
> 中使用最频繁的一列放在最左边。
>
> 最左前缀匹配原则，非常重要的原则，MySQL会一直向右匹配直到遇到范围查
>
> 询(\>、\<、between、like)就停止匹配，比如a = 1 and b = 2 and c \> 3
> and
>
> d = 4 如果建立(a,b,c,d)顺序的索引，d是用不到索引的，如果建立
>
> (a,b,d,c)的索引则都可以用到，a,b,d的顺序可以任意调整。
>
> =和in可以乱序，比如a = 1 and b = 2 and c = 3 建立(a,b,c)索引可以任
>
> 意顺序，MySQL的查询优化器会帮你优化成索引可以识别的形式。
>
> 78
>
...
>
> <img src="./2lzqf0db.png"
> style="width:5.76319in;height:5.77014in" />到磁盘相应数据，这也就是为什么索引不在key
> buffer命中时，速度慢的
>
> 原因。
>
...
>
> MySQL在得到一个执行请求后，会首先去查询缓存 中查找，是否执行过这条
>
> SQL语句，之前执行过的语句以及结果会以key-value对的形式，被直接放在
>
> 内存中。key是查询语句，value是查询的结果。
>
...
> 就会被放入查询缓存中。
>
> 可以看到，如果查询命中缓存，MySQL不需要执行后面的复杂操作，就可以直
>
> 接返回结果，效率会很高。
>
> <img src="./jnf43oc3.png"
> style="width:5.76319in;height:5.77014in" />**23、MySQL** **分析器**
>
...
>
> 话，就会返回没有权限的错误。如果有权限，就打开表继续执行。打开表的时
>
> 80
>
> 候，执行器就会根据表的引擎定义，去使用这个引擎提供的接口。对于有索引
>
> 的表，执行的逻辑也差不多。
>
> **26、什么是临时表，何时删除临时表？**
>
> <img src="./qfx2iri5.png"
> style="width:5.76319in;height:5.77014in" />什么是临时表?MySQL在执行SQL语句的过程中
> 通常会临时创建一些存储中间
>
...
>
> <img src="./rpeofywt.png"
> style="width:5.76319in;height:5.77014in" />析，进行优化。
>
> ⚫ 当你的 SELECT 查询语句只需要使用一条记录时，要使用 LIMIT 1。不要
>
> 直接使用SELECT\*，而应该使用具体需要查询的表字段，因为使用EXPLAIN
>
...
>
> <img src="./n1fwsop4.png"
> style="width:5.76319in;height:5.77014in" /><img src="./aqx2gxfq.png"
> style="width:5.70292in;height:2.2875in" />不符合条件的数据行直接显示NULL。
>
...
>
> <img src="./vm25tjsb.png"
> style="width:5.76319in;height:5.77014in" />子节点上，而MylSAM的表数据和索引数据是分开的。
>
...
>
> Unix工具软件、应用程序和网络协议。它支持32位和64位硬件。Linux继承
>
...
>
> <img src="./tuiakebo.png"
> style="width:5.76319in;height:5.77014in" />款对源码实行知识产权保护的传统商业软件，使用需要付费授权使用。
>
...
> 都附带其他功能，所有代码都可以免费下载。
>
> <img src="./nwr2mepo.png"
> style="width:5.76319in;height:5.77014in" />**5、Linux** **的体系结构**
>
...
> 作命令参数分隔符，\是目录分隔符
>
> ⚫ DOS遵循命名文件中的约定，即8个字符的文件名后跟一个点，扩展名为3
>
> 个字符。BASH没有遵循这样的惯例。
>
> 87
...
> ⚫ 引导 Linux 内核
>
> <img src="./wywqf2pn.png"
> style="width:5.76319in;height:5.77014in" />⚫ 运行第一个进程 init
> (进程号永远为 1 )
>
> ⚫ 进入相应的运行级别
>
...
>
> <img src="./3ojzwazb.png"
> style="width:5.76319in;height:5.77014in" />去。
>
...
> 中，以便它可以开始运行。
>
> <img src="./we2vt04y.png"
> style="width:5.76319in;height:5.77014in" />**15、什么是** **BASH？**
>
...
> 界面相比，图形用户界面有许多优点。
>
> <img src="./3h4xsvfd.png"
> style="width:5.76319in;height:5.77014in" />**18、开源的优势是什么？**
>
...
> **1、缓存如何实现高性能？**
>
> <img src="./3owcbvpn.png"
> style="width:5.76319in;height:5.77014in" /><img src="./5llffkae.png"
> style="width:5.70319in;height:2.37361in" /><img src="./tqgfzzzi.png"
> style="width:5.70361in;height:2.85972in" />**2、缓存如何实现高并发？**
>
> 92
>
...
> Memcached稍有逊色
>
> <img src="./bowzwt4t.png"
> style="width:5.76319in;height:5.77014in" />Memcached没有原生的集群模式，Redis官方支持Redis
> Cluster集群模式
>
> **4、用缓存可能出现的问题**
>
...
> 一般建议不要超过 5 分钟。
>
> <img src="./jqpjccse.png"
> style="width:5.76319in;height:5.77014in" />方案二，BloomFilter
> 布隆过滤器。
>
...
>
> <img src="./wlykjgge.png"
> style="width:5.76319in;height:5.77014in" />加载数据并回设到缓存，但是这个时候大并发的请求可能会瞬间
> DB 压垮。
>
...
>
> <img src="./ugb5qx22.png"
> style="width:5.76319in;height:5.77014in" />的。
>
...
>
> 在执行。单核处理器可以做到并发。比如有两个进程A 和B，A 运行一个时间
>
> 97
>
> 片之后，切换到B，B 运行一个时间片之后又切换到A。因为切换速度足够
>
...
> 是物理上的多个进程同时进行。
>
> <img src="./rcx1xw3d.png"
> style="width:5.76319in;height:5.77014in" />**4、进程与线程的切换流程？**
>
> 进程切换分两步：
>
...
>
> <img src="./a2zr1uxy.png"
> style="width:5.76319in;height:5.77014in" />关系的进程间通信；命名管道以磁盘文件的方式存在，可以实现本机任意两个
>
> 进程通信。
>
...
> **8、线程同步的方式有哪些？**
>
> <img src="./tc1iu3vt.png"
> style="width:5.76319in;height:5.77014in" />**临界区：**当多个线程访问一个独占性共享资源时，可以使用临界区对象。拥有
>
...
>
> 由内核实现。比如英特尔i5-8250U是4核8线程，这里的线程就是内核级线程
>
...
> 用户级线程的存在。
>
> <img src="./cexrco0k.png"
> style="width:5.76319in;height:5.77014in" />**10、什么是临界区，如何解决冲突？**
>
...
>
> <img src="./kaq355qu.png"
> style="width:5.76319in;height:5.77014in" />可能鸵鸟觉得看不到危险也就没危险了吧。跟掩耳盗铃有点像。
> ⚫ 检测死锁并且恢复。
>
> ⚫ 仔细地对资源进行动态分配，以避免死锁。
>
...
> 性就不能得到保证。
>
> <img src="./4lxexvli.png"
> style="width:5.76319in;height:5.77014in" /><img src="./dmc4l3hz.png"
> style="width:5.70292in;height:2.37986in" />⚫
...
>
> <img src="./lrxbeacs.png"
> style="width:5.76319in;height:5.77014in" />占处理器等。
>
...
> 106
>
> <img src="./gjhwhueo.png"
> style="width:5.76319in;height:5.77014in" /><img src="./thueisfq.png"
> style="width:5.70417in;height:3.49792in" /><img src="./qdbtiv5z.png"
> style="width:5.70333in;height:2.71736in" />**15、什么是分段？**
>
...
>
> 段的长度是不一样的，而且每个段内部都是从0开始编址的。由于分段管理
>
...
>
> <img src="./gn0basyc.png"
> style="width:5.76319in;height:5.77014in" />使程序和数据可以被划分为逻辑上独立的地址空间并且有助于共享和保
>
> 护。
>
> **17、什么是交换空间？**
...
>
> 最老的页面。它将整个环形链表的每一个页面做一个标记，如果标记是0，
>
> 那么暂时就不会被替换，然后时钟算法遍历整个环，遇到标记为1的就替
>
> 换，否则将标记为0的标记为1。
>
> **18、什么是缓冲区溢出？有什么危害？**
...
>
> <img src="./c0tdialq.png"
> style="width:5.76319in;height:5.77014in" />存。
>
> **20、讲一讲** **IO** **多路复用？**
>
...
>
> 数量不为0。但是硬链接有限制，它不能跨越文件系统，也不能对目录进行
>
> 链接。
>
> 110
...
>
> 上，可以理解为Windows的快捷方式。当源文件被删除了，链接文件就打
>
> 不开了。因为记录的是路径，所以可以为目录建立符号链接。
>
> **22、中断的处理过程?**
>
> <img src="./ssp3idla.png"
> style="width:5.76319in;height:5.77014in" />⚫
...
>
> 段，该报文段的 SYN 标志位会被置1，所以把它称为 SYN 报文段。这时客户端
>
> 会选取一个初始序列号（假设为
> client_num），并将此编号放置在序号字段中。
>
...
>
> 允许连接的确认报文。在允许连接的报文中，SYN 标志位仍被置为1，确认号字
>
...
>
> <img src="./wb0kvogb.png"
> style="width:5.76319in;height:5.77014in" />server_num +
> 1的值，并且这个报文段可以携带数据。一旦完成这3个步骤，
>
...
>
> 器发送SYN 报文段。服务器接收到 SYN 报文段后会为该TCP分配缓存和变量，
>
> 如果攻击分子大量地往服务器发送 SYN
> 报文段，服务器的连接资源终将被耗尽，
>
...
> 序列号响应给客户端。
>
> 如果客户端是正常建立连接，将会返回一个确认字段为 cookie + 1的报文段。
>
...
> 会向服务器发送一个特殊的报文段，
>
> <img src="./hcwczeqw.png"
> style="width:5.76319in;height:5.77014in" />该报文段的 FIN
> 标志位会被置1，接着服务器会向客户端发送一个确认报文段。
>
> 然后服务器也会客户端发送一个 FIN 标志位为1的终止报文段，随后客户端回
>
...
>
> <img src="./vxtipty3.png"
> style="width:5.76319in;height:5.77014in" />据。
>
...
> ⚫ Producer（生产者）
>
> <img src="./udzu2jja.png"
> style="width:5.76319in;height:5.77014in" />⚫ Consumer（消费者）
...
>
> **5、Kafka** **的多分区（Partition）以及多副本（Replica）机制有**
>
> **什么好处呢？**
>
> 115
...
>
> <img src="./3l2gwe23.png"
> style="width:5.76319in;height:5.77014in" />**6、Zookeeper** **在**
> **Kafka** **中的作用知道吗？**
>
...
> 有发送过去。
>
> <img src="./zgc2kt2p.png"
> style="width:5.76319in;height:5.77014in" />所以，我们不能默认在调用send方法发送消息之后消息发送成功了。为了确定
>
...
> 能太久。
>
> **10、producer** **是否直接将数据发送到** **broker** **的**
> **leader（主节**
>
> **点）？**
>
> <img src="./33zrkigx.png"
> style="width:5.76319in;height:5.77014in" />producer 直接将数据发送到
> broker 的 leader(主节点)，不需要在多个节点
>
> 进行分发，为了
>
...
>
> <img src="./pupnsftg.png"
> style="width:5.76319in;height:5.77014in" />如果大小大于 1g
> 时，会滚动一个新的 segment 并且以上一个 segment 最后
>
> 一条消息的偏移量命名。
>
...
>
> <img src="./neokm1f4.png"
> style="width:5.76319in;height:5.77014in" />**分布式**
>
> **1、分布式服务接口的幂等性如何设计？**
>
> 所谓幂等性，就是说一个接口，多次发起同一个请求，你这个接口得保证结果
>
> 是准确得。比如不能多扣款。不能多插入一条数据，不能将统计值多加了1，
>
> 这就是幂等性。
>
> 其实保证幂等性主要是三点：
...
> 并发性。
>
> <img src="./0tun2vez.png"
> style="width:5.76319in;height:5.77014in" />**3、说说** **ZooKeeper**
> **一般都有哪些使用场景？**
>
...
>
> <img src="./kcpvh3p3.png"
> style="width:5.76319in;height:5.77014in" />⚫ Spring Session + Redis
>
...
> **8、MySQL** **如何做分布式锁？**
>
> <img src="./h3p4xzvw.png"
> style="width:5.76319in;height:5.77014in" />**方法一：**
>
...
>
> 求插入了一条ID为1的数据，其他想要进行插入数据的并发请求必须等第一次
>
> 请求执行完成后删除这条ID为1的数据才能继续插入，实现了分布式锁的功
>
> 能。
>
> **方法二：**
...
> ⚫ **京东** **SharkLock**
>
> <img src="./vrtckckn.png"
> style="width:5.76319in;height:5.77014in" />SharkLock是基于Redis实现的分布式锁。锁的排他性由SETNX原语实现，使
>
...
> 成。
>
> 在ZooKeeper创建EPHEMERAL_SEQUENTIAL 节点视为加锁，节点的
>
...
>
> <img src="./hev4j2rj.png"
> style="width:5.76319in;height:5.77014in" />CAP法则主要满足的是C和P法则。
>
...
>
> 是一个非常具有误导性质的说法，而且在CAP理论诞生12年之后，CAP之父也
>
> 在2012年重写了之前的论文。
>
> 当发生网络分区的时候，如果我们要继续服务，那么强一致性和可用性只能2
>
> 选1。也就是说当网络分区之后P是前提，决定了P之后才有C和A的选择。
>
> 也就是说分区容错性（Partition tolerance）我们是必须要实现的。
>
> **11、请讲一下你对** **BASE** **理论的理解**
>
> BASE理论由eBay架构师Dan Pritchett提出，在2008年上被分表为论文，并
>
...
>
> <img src="./bnwuvgvg.png"
> style="width:5.76319in;height:5.77014in" />**12、分布式与集群的区别是什么？**
>
...
> **最终一致性**
>
> <img src="./gi5tvb5o.png"
> style="width:5.76319in;height:5.77014in" />强调的是系统中所有的数据副本，在经过一段时间的同步后，最终能够达到一
>
...
>
> ⚫ 所有节点都采用预写式日志，且日志被写入后即被保持在可靠的存储设备
>
> 上，即使节点损坏不会导[致日志数据的](https://cloud.tencent.com/solution/cloudlog?from=10680)消失。
>
...
<img src="./sru05iz0.png"
style="width:5.76319in;height:5.77014in" /><img src="./koifrekx.png"
style="width:5.76389in;height:3.24236in" /><img src="./5mpvlnju.png"
style="width:5.76389in;height:3.24236in" />

> **Go** **进阶训练营**
>
> 131

<img src="./31caixoa.png"
style="width:5.76319in;height:5.77014in" /><img src="./12mzarwz.png"
style="width:5.76389in;height:6.00069in" /><img src="./wolmkbgx.png"
style="width:5.76389in;height:3.24236in" />

> 132
>
> <img src="./eak1dwcq.png"
> style="width:5.76319in;height:5.77014in" /><img src="./lrjkjdrh.png"
> style="width:5.76389in;height:3.24236in" /><img src="./n5bmblqs.png"
> style="width:5.76389in;height:3.38889in" />Go
> 进阶训练营课程大纲[：http://gk.link/a/10wfO](http://gk.link/a/10wfO)
>
> Go
> 进阶训练营核心实战项目[：http://gk.link/a/10pom](http://gk.link/a/10pom)
>
> 133